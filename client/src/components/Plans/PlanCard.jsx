...
const PlanCard = ({ plan, onEdit, onDelete, onContribute }) => {
    ...
    return (
        <div className={`p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 ${color.bg} dark:bg-opacity-90`}>
            ...
            <div className="flex justify-between text-sm mb-1">
                <span className="font-medium text-gray-800 dark:text-gray-200">{formatCurrency(plan.current_amount)}</span>
                <span className="text-gray-600 dark:text-gray-400">из {formatCurrency(plan.target_amount)}</span>
            </div>
            <div className="w-full h-2 bg-gray-200 dark:bg-gray-600 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${progress}%` }} />
            </div>
            {remaining > 0 ? (
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-2">Осталось {formatCurrency(remaining)}</p>
            ) : (
