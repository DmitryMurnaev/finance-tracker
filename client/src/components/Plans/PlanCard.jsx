    ...
    return (
        <div className={`p-4 rounded-xl shadow-sm border border-gray-100 ${color.bg}`}>
            ...
            <div className="flex justify-between text-sm mb-1">
                <span className="font-medium">{plan.current_amount.toLocaleString('ru-RU')} ₽</span>
                <span className="text-gray-600">из {plan.target_amount.toLocaleString('ru-RU')} ₽</span>
            </div>
            <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${progress}%` }} />
            </div>
            {remaining > 0 ? (
                <p className="text-xs text-gray-600 mt-2">Осталось {remaining.toLocaleString('ru-RU')} ₽</p>
            ) : (
